...
int main() {
  ...
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "uri",
                                   "grpc+tcp://localhost:8181", &error));
  CHECK_ADBC(AdbcDatabaseSetOption(
      &database, "adbc.flight.sql.authorization_header",
      "Bearer YOUR_AUTH_TOKEN", &error));
  CHECK_ADBC(AdbcDatabaseSetOption(
      &database, "adbc.flight.sql.rpc.call_header.database", "_internal",
      &error));
  ...
  CHECK_ADBC(AdbcStatementSetSqlQuery(
      &statement, "SELECT * FROM information_schema.tables", &error));
  CHECK_ADBC(
