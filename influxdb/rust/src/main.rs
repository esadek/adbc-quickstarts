...
fn main() {
    ...
    let opts = [
        (OptionDatabase::Uri, "grpc://localhost:8181".into()),
        (
            OptionDatabase::Other("adbc.flight.sql.authorization_header".to_string()),
            "Bearer YOUR_AUTH_TOKEN".into(),
        ),
        (
            OptionDatabase::Other("adbc.flight.sql.rpc.call_header.database".to_string()),
            "_internal".into(),
        ),
    ];
    let db = driver
    ...
    statement
        .set_sql_query("SELECT * FROM information_schema.tables")
        .unwrap();
