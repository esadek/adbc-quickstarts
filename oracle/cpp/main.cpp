...
int main() {
  ...
  CHECK_ADBC(AdbcDatabaseSetOption(&database, "driver", "oracle", &error));
  CHECK_ADBC(AdbcDatabaseSetOption(
      &database, "uri", "oracle://system:password@localhost:1521/FREEPDB1",
      &error));
  ...
  CHECK_ADBC(
      AdbcStatementSetSqlQuery(&statement, "SELECT version FROM v$instance;", &error));
  CHECK_ADBC(
