...
fun main() {
    ...
    params["uri"] = "databricks://<server-hostname>:<port-number>/<http-path>?authType=OauthU2M"

    // Authenticate using OAuth M2M (client credentials authentication)
    // params["uri"] = "databricks://<server-hostname>:<port-number>/<http-path>?authType=OAuthM2M&clientID=<client-id>&clientSecret=<client-secret>"

    // Authenticate using a personal access token
    // params["uri"] = "databricks://token:<personal-access-token>@<server-hostname>:<port-number>/<http-path>"
